...
class GitHubIntegrationClient {
     ...
     */
    initializeOctokit() {
        // Octokit が読み込まれていることを確認 (UNPKG CDN)
        if (typeof window.Octokit === 'undefined') {
            console.error('Octokit.js is not loaded. Please check the CDN link.');
            console.log('Available globals:', Object.keys(window).filter(k => k.toLowerCase().includes('octokit')));
            return;
        }

        try {
            // UNPKG CDN は window.Octokit として直接公開
