...
**ユーザーストーリー:** レビュー指摘テーブルを閲覧するユーザーとして、各列の内容を適切な幅で読みやすく表示したい。そうすることで、指摘内容を効率的に確認できる。

#### 受入基準

1. WHEN レビュー指摘テーブルを表示する THEN システムは「内容」列に十分な幅を割り当て、文章を読みやすく表示しなければならない
2. WHEN テーブルを表示する THEN システムは各列の重要度に応じて適切な幅比率を設定しなければならない
3. WHEN 「内容」列のテキストが長い場合 THEN システムは適切な行の高さを保ちながら、テキストを折り返して表示しなければならない
4. WHEN テーブルを表示する THEN システムは分類フィールド（プロセス、ドキュメント種別、カテゴリなど）には最小限の幅を割り当てなければならない
5. WHEN レスポンシブ表示が必要な場合 THEN システムは小さな画面でも「内容」列の可読性を維持しなければならない
